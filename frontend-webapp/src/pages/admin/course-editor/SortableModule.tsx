    ...
    return (
        <div ref={setNodeRef} style={style} className="min-w-0 space-y-1">
            <div
                ...
                {...listeners}
                className={cn(
                    "flex min-w-0 cursor-grab items-start justify-between gap-3 rounded-xl border p-3 transition-colors duration-200 active:cursor-grabbing sm:p-4",
                    isExpanded
                        ? "bg-card border-border shadow-sm z-10 relative"
                        : "bg-muted/50 border-border/60 opacity-90"
                )}
            >
                <div className="flex min-w-0 flex-1 items-start gap-3 pt-0.5" onClick={(e) => { e.stopPropagation(); onToggle(); }}>
                    <span className={cn("material-symbols-outlined shrink-0 text-2xl leading-none", isExpanded ? "text-primary" : "text-muted-foreground")} style={isExpanded ? { fontVariationSettings: "'FILL' 1" } : {}}>
                        {isExpanded ? 'folder_open' : 'folder'}
                    </span>
                    <div className="min-w-0 flex-1 space-y-1">
                        <h3 className={cn("min-w-0 whitespace-normal break-words text-base font-semibold leading-snug", !isExpanded && "text-muted-foreground")}>
                            {module.title}
                        </h3>
                        {module.is_vip && (
