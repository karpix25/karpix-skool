            ...
            <div className="p-6 sm:p-8 space-y-6">
                ...
                <div className="space-y-6">
                    <div className="space-y-2">
                        <Label className="text-[10px] font-black text-muted-foreground ml-1">Название</Label>
                        <Input
                    ...
                    <div className="space-y-4">
                        <Label className="text-[10px] font-black text-muted-foreground ml-1">Стратегия доступа</Label>
                        <div
                    ...
                    {moduleForm.unlock_type !== 'immediate' && (
                        <div className="space-y-2 animate-in slide-in-from-top-2 duration-300">
                            <Label className="text-[10px] font-black text-muted-foreground ml-1">
                                ...
                                <SelectContent className="rounded-lg border-border/60 shadow-md p-1">
                                    {moduleForm.unlock_type === 'level_based' ? (
                                        [1, 2, 3, 5, 10, 20].map(lv => (
                                            <SelectItem key={lv} value={lv.toString()} className="rounded-lg h-10 font-bold text-[11px]">
                                                Уровень {lv}
                                            </SelectItem>
                                        ))
                                    ) : (
                                        <>
                                            ...
                                            {[1, 2, 3].map(m => (
                                                <SelectItem key={`m${m}`} value={`${m}m`} className="rounded-lg h-10 font-bold text-[11px]">
                                                    {m} {m === 1 ? 'месяц' : 'месяца'}
                                                </SelectItem>
                                            ))}
                                        </>
                ...
                <div className="flex flex-col gap-2 pt-4">
                    <Button onClick={onSave} disabled={!moduleForm.title} className="h-12 bg-primary text-white hover:bg-primary/90 rounded-lg font-bold text-[10px]">
                    ...
                    {editingModule && (
                        <Button
                            variant="ghost"
                            className="h-12 text-destructive hover:text-destructive hover:bg-destructive/10 rounded-lg font-bold text-[9px]"
                    ...
                    <Button
                        ...
                        className="h-10 text-muted-foreground/60 hover:text-foreground font-bold text-[10px] transition-colors"
                    >
                        Отмена
                    </Button>
                </div>
