...
export const ActionOverlay: React.FC<ActionOverlayProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div 
      className="fixed inset-0 z-50 flex flex-col justify-end items-center bg-black/60 backdrop-blur-[2px] px-4 pb-12 transition-all duration-300 animate-in fade-in"
      onClick={onClose}
    >
      <div 
        className="w-full max-w-md bg-white dark:bg-slate-900 rounded-3xl overflow-hidden shadow-2xl border border-white/10 mb-6 transform transition-transform duration-300 translate-y-0 animate-in slide-in-from-bottom-10"
      ...
      >
        ...
        <div className="px-2 pb-6">
          {actionItems.map((item) => (
            <button
              key={item.id}
              className="w-full flex items-center gap-4 p-4 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-colors text-left group"
              onClick={() => {
                console.log(`Action clicked: ${item.title}`);
                onClose();
              }}
            >
              <div className={`w-12 h-12 rounded-xl ${item.bgClass} flex items-center justify-center ${item.colorClass} group-active:scale-90 transition-transform`}>
                {item.isSvg ? (
      ...
      {/* Floating Action Button (Internal) */}
      <button 
