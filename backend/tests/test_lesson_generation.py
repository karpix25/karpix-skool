...
async def test_process_lesson_job_marks_unanswerable_source_and_stores_raw_response(monkeypatch):
    ...
    async def allow_ai(*_args, **_kwargs):
        return 1
    monkeypatch.setattr(jobs, "reserve_ai_generation_execution", allow_ai)
...
@pytest.mark.asyncio
async def test_process_lesson_job_persists_created_course_notebook_on_parse_error(monkeypatch):
...
def test_lesson_generation_routes_create_and_read_job(monkeypatch):
    ...
    monkeypatch.setattr(lesson_generation, "ensure_ai_generation_request", fake_ensure_tenant_access)
